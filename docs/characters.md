...
**Fantasy:** The assassin. Sprint Dash in, land a kill, Radar Pulse to find another target, then disappear before anyone can respond. Carrot rewards players who study enemy positions and punish overextension.
...
- **Radar Pulse** (`Ability_RadarPulse`): `Physics.OverlapSphere` plus `SetOutlineVisible()` on hit `PlayerController`s; 15m radius, 2.5s.
...
- **Backstab Momentum** (`MomentumPassive_Backstab`): the rear-arc test lives in the spine. `GameExtensions.IsInRearArc` uses dot `< -0.1` (`CarrotBackstabDot`), forwarded to `MomentumController.RegisterKill(isBackstab)` for the +2-tier grant.
:::
...
- Radar Pulse before entering a building so you know the room before you walk in
- Tier 3 Carrot with the Backstab passive grants +2 tier to your killer if you die, so you stay dangerous even when hunted
...
**Fantasy:** The room clearer. Walk into a chokepoint, detonate Spice Burst to slow everyone, lay a Heat Trail behind you to cut off retreat, then shotgun anything that survives. High floor and high ceiling: beginners can get kills, experts dominate close range.
...
- **Heat Trail** (`Ability_HeatTrail`): fire segments at the feet, each a `DamageZone` trigger; 20 DPS, 1m wide (0.5m radius), 3s.
...
| Potato | Two Jalapeños can overwhelm a Potato | 1v1 vs. Potato: Starch Armor absorbs one full shotgun burst, so use Slam to break it |
...
**Fantasy:** The force multiplier. Broccoli does nothing flashy alone, but with good positioning it keeps the team alive and in fights longer than the enemy expects. Mechanically demanding: it requires spatial awareness of allies at all times.
...
|---|---|---|
| Carrot | Spore Cloud negates Carrot's positioning intel (Radar Pulse reveals through it, but Spore also blinds Carrot mid-dive) | Carrot's Dash into close range is hard to survive; use Leaf Shield as panic cover |
...
| Potato | Broccoli Regen Aura makes Potato nearly unkillable if stacked | No damage-dealing combo with Potato; dependent on team for kills |
...
| All | Leaf Shield on a capture zone makes holding it much easier: 80 HP of free cover that respawns on cooldown |
...
|---|---|---|
| Carrot | Carrot can't burst down 140 HP + Starch Armor before being slammed | Carrot's dash away from Earthen Slam is instant; it rarely lands on mobile targets |
| Jalapeño | Starch Armor absorbs the first shotgun blast, buying time to slam | Jalapeño Spice Burst slows Potato to a crawl and wins the timer game |
| Broccoli | Potato is safe from Broccoli's burst AR in sustained fire; it outlasts | Broccoli Spore Cloud + regen means the enemy Broccoli heals teammates faster than Potato kills them |
