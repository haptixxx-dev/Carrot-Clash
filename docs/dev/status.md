...
| Character data | [Characters](/characters) | <span class="cc-status partial">Partial</span> | `CharacterDataSO` exists; the 4 assets are **generated by the editor menu, then committed**. |
...
- **Data assets** (4 `CharacterDataSO`, 5 `WeaponDataSO`, 16 `AbilityDataSO`, `MomentumConfigSO`)
  are **generated from the docs**, then committed. Run `Carrot Clash → Generate Default Data Assets` once if they're not yet in your checkout,
