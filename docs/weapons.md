...
**Design note:** The Nub rewards consistent aim at close-mid range. Its fast fire rate pairs with Carrot's dash mobility: burst in, empty mag, dash out. It's weak beyond 25m, so Carrot should not be taking long duels.
...
**Design note:** The Pepper Blaster is the highest-risk, highest-reward weapon. Jalapeño is terrifying in close range and nearly useless beyond 15m. This backs Jalapeño's role as a room-clearing brawler: get in, slam and burst, get out.
...
**Design note:** The Stem is not a dueling weapon. Broccoli wins firefights through positioning and support, not raw DPS. The burst pattern is forgiving for moderate-skill players (spray control between bursts) without being oppressive. Range advantage compensates for low DPS.
...
**Design note:** The Root Cannon is a sustained-fire suppression weapon. The big magazine means Potato rarely reloads during a push, and Starch Armor covers the long reload. The movement penalty keeps Potato a stationary anchor: disengage to reload, not to win duels.
...
**Design note:** The Pip is a backup, not a primary alternative. The 2× headshot multiplier makes it a real threat at close range for skilled players (3 headshots = 168 damage, which kills any class). Switching to pistol is faster than reloading (0.2s swap vs. 1.8-3.5s reload).

::: info Code reconciliation: swap behaviour
`WeaponController.SwapWeapon()` toggles primary ↔ pistol with the documented `WeaponSwapTime = 0.2f` gate. The current build has one simplification: each swap-in refills the incoming weapon's magazine to full rather than persisting per-weapon ammo state across swaps. This keeps the pistol a clean panic option. Persistent per-weapon mags are an open tuning decision if it proves too forgiving.
:::
...
- **Recoil** is <span class="cc-status partial">Partial</span>: `WeaponDataSO.recoilPattern` (deterministic, learnable per-weapon arrays) and per-shot `cameraKick` exist and are applied via `WeaponController.ApplyRecoil()`, but the pattern is currently only consumed as a minimal camera nudge. Full pitch/yaw walk-up is editor-tuning work. Hit-stop and hit markers route through the feedback layer (`Assets/_Game/UI/Feedback/`, `HitMarkerUI`).
:::
