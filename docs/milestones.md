...
::: warning Data assets are generated, then committed
`CharacterDataSO`/`WeaponDataSO`/`MomentumConfigSO` instances are produced by the editor menu **Carrot Clash → Generate Default Data Assets** (`DataAssetGenerator`), then committed to the repo. The generator is re-runnable and overwrites in place so GUIDs stay stable. If your checkout doesn't have them yet, run the menu once and commit the result; validate with `DataValidator`.
:::
...
- [x] `CharacterDataSO` assets for all 4 classes: generated by editor menu, then committed
