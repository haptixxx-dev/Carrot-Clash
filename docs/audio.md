...
**Post-match:** Brief 3-note fanfare for win (upbeat), 1-note subdued cue for loss. Then silence — the XP animation plays in clean audio space so numbers feel more impactful. (`MusicDirector.StopMatchMusic()` cuts all layers on `MatchEnd`/`PostMatch`.)
...
- **Intensity crossfade:** `intensityFadeSpeed = 0.2f`/sec → a ~5s full fade, matching the doc's "cross-fade in over 5s".
...
- **Surface detection:** `Physics.RaycastNonAlloc` straight down from the feet (origin +0.4m, probe length 1.5m); the nearest non-self collider's `PhysicsMaterial` **name** is resolved to a `SurfaceType` by `FootstepBank` (exact case-insensitive match → substring containment → default `Stone`). So designers can name a material `Courtyard_Stone` and it still resolves.
...
- **Carrot Silent Steps passive:** steps are **skipped** (not just volume 0) when a Carrot moves below `SprintSpeed * SilentStepsSpeedFraction`, with `SilentStepsSpeedFraction = 0.5f` — i.e. < 50% sprint, matching the design.
...
<span class="cc-status built">Implemented</span> `AmbientZone.cs` — a trigger volume per zone that starts/stops one library loop for the local player. Loop clips + placed trigger volumes are editor-pending.
...
<span class="cc-status built">Implemented</span> `AudioManager.cs` (singleton, `DontDestroyOnLoad`). The `AudioMixer` graph + an `AudioLibrary` asset must be wired in the editor; until then it runs silent.
...
- Category buses via Unity `AudioMixer`. The code wires four serialized groups: **Music**, **SFX**, **UI**, **Ambience**. 2D one-shots → UI group, 3D one-shots and spatial loops → SFX group, non-spatial loops → Ambience group. (The finer per-category SFX sub-buses — Footsteps / Weapons / Abilities — are an editor-side mixer-graph detail, not separate code groups.)
...
- Music: one looping `AudioSource` per layer (`MusicBase` / `MusicIntensity` / `MusicTier3`); crossfades are done by lerping layer volume + the matching exposed mixer float (see `SetMusicParameter`), not snapshot transitions.
- Volume: master/music/SFX/voice volumes come from `SettingsService` and are pushed to exposed mixer floats (`MasterVolume`, `MusicVolume`, `SfxVolume`, `VoiceVolume`) on settings change, converted linear→dB.
- Footstep surface detection: `PhysicsMaterial` name → `SurfaceType` enum → `footstep_{surface}` key, via `FootstepBank` ScriptableObject.
...
<span class="cc-status pending">Editor-pending</span> The runtime is done; this is the remaining content checklist. Once produced, clips drop into the `AudioLibrary` (under the keys documented above) and `FootstepBank` assets — no code changes required.
