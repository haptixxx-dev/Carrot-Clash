...
An oversized open-air market fused with a commercial kitchen. Picture a farmer's market after a food fight: bright awnings, crates of produce stacked as cover, vendor stalls that can be blown apart, and an elevated central stage built for cooking demonstrations (now the match's deciding zone).

The environment is mechanical. Cover objects are themed to the class colours (orange crates for Carrot territory, red pepper barrels for Jalapeño's domain) to help players orient themselves. Every part of the map supports a specific combat scenario.
...
- Multi-capper speedup. Each extra teammate inside the radius adds `perPlayerSpeedup` (0.5 = +50% of base rate, diminishing), clamped to `maxSpeedup` (2.5×). So "Capture time (1 player)" below is the *solo* time; a stacked team captures up to 2.5× faster.
...
- Flat (horizontal) distance drives occupancy, so the raised Zone C platform is judged fairly regardless of height.
...
> Destructible stalls map to `DestructibleCover.cs`, whose default HP pool is `80`, matching the design. The component shatters (self-destructs, plays `impact_surface` SFX, optional VFX) at 0 HP.
...
**Capture point placement:** Center of the courtyard. The capturer is visible from all 3 lanes, so there's no fully safe capture position, but cover is close.
...
**Capture point placement:** Behind the main counter, partially sheltered but accessible from the front entry, back entry, and mezzanine. No single dominant hold angle.

**Sightlines:** Max 12m clear sight; the rest is cornered. Favours close-range classes heavily.
...
**Class affinity:** Mixed. The open platform rewards Carrot mobility and Jalapeño AOE, but Potato plus Broccoli can anchor the central column nearly indefinitely.

**Tactical note:** The platform is visible from Zone A's rooftop line of sight. Carrot with Radar Pulse can see who's contesting Zone C from Zone A, which enables info plays.

**Barrier unlock:** Physical barrier objects lower at second 300; a global audio announcement ("The Stage is open!") plays for all players; the HUD flashes the Zone C icon.

> Code: a `CaptureZone` with `lockUntilMatchTime = 300` (`GameConstants.ZoneCUnlockTime`) stays inert until `GameModeManager.ElapsedTime` crosses 300s, then fires `GameEvents.RaiseZoneCUnlocked()` (HUD and audio listen for it). `scoreTickRate` is `2` here vs `1` for A/B, confirmed in both doc and code. The physical barrier mesh and the unlock SFX/announcement are editor-pending; the event that triggers them is wired.

> Info-play note (Carrot from Zone A): `Ability_RadarPulse` ships with a 15m scan radius and 2.5s reveal, so the rooftop-sightline call-out above holds up against the actual ability values.
...
| Underground Cellar | Zone B basement ↔ Zone C underside stairs | ~10s | Low | Slow; sound-dampened (quiet footsteps here regardless of class); ideal for Potato or a surprise push |

::: details Code note: footsteps & sightline denial
The cellar's stealth feel rides on the footstep system. `SurfaceType.Dirt` (`GameEnums.cs`) is the cellar's surface category, resolved per-collider by `FootstepBank`. The "quiet regardless of class" claim is a design intent for the surface/material setup; there's no route-based code that further dampens footsteps, and Carrot's separate `Passive_SilentSteps` is the class-level stealth.
...
### Spawn safety rule
If a spawn position has an enemy within 10m, the spawning player is redirected to the nearest safe teammate position. If no safe position exists, the spawn delay extends by 1s and the player spawns at the safe fixed position with 3s invulnerability.

The shipped `SpawnManager.ChooseSpawn` runs this exact cascade:

- Respawn delay: `RespawnTime = 4s` before spawn selection begins.
- Try a fixed spawn: first uncontested team spawn point, where "contested" means an enemy within `SpawnContestRadius = 10m`.
...
The doc says "nearest safe teammate position" without bounding it; the code caps that search at 30m (`SafeTeammateRadius`). This is a code-only constant that tightens the rule so you can't be flung across the map. Everything else (10m contest radius, +1s/3s fallback, 2s normal invuln) matches the design verbatim.
:::
...
**Zone C unlock audio:** A dramatic kitchen bell rings once, crowd noise surges, and a HUD SFX plays. A clear signal that the stakes have escalated.
