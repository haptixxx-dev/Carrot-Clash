...
The gameplay spine is <span class="cc-status built">Implemented</span> in code: 97 C# scripts under `Assets/_Game/` cover player core, the momentum tier system, all 4 classes + 16 ability behaviours, the match state machine + scoring, spawns, the full HUD, menus, bots, audio, and progression — all verified against this design doc.
...
Two deltas to flag:
- **Networking is not live.** NGO + Relay are designed and the network mirrors are written, but the packages are not installed — that code compiles only behind `#if NETCODE_PRESENT`. Current play is single-player vs bots.
